...
class DataTransformation:
    ...
    def get_data_transformation_object(self):
        ...
        try:
            ...
            categorical_cols = ['Airline', 'Duration', 'Total_Stops', 'Additional_Info',
    ...
    def initiate_data_transformation(self, train_path,test_path):
        try:
            ...
            train_final=pd.concat([train_df,df],axis=1)
            train_final.drop(['Source','Destination','Route'], axis=1,inplace=True)

            df=pd.DataFrame(encoder.fit_transform(test_df[['Source','Destination']]).toarray(),columns=encoder.get_feature_names_out())
            test_final=pd.concat([test_df,df],axis=1)
            test_final.drop(['Source','Destination','Route'], axis=1,inplace=True)
